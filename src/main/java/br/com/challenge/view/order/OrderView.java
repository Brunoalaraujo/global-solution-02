...
public class OrderView {
    ...
    public static void execute() throws SQLException {
        ...
        try {
            do {
                menu();
                choice = readChoice();
                handleMenuOption(choice, orderDAO, walletDAO);
            } while (choice != 0);
        } finally {
    ...
    private static void handleMenuOption(int choice, OrderDAO orderDAO, WalletDAO walletDAO) {
        try {
            switch (choice) {
                case 1 -> CreateOrderView.execute(sc, orderDAO, walletDAO);
